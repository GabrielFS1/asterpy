...
def threshold_adjust_window(file, path, dir, index, func):
    ...
    def drag_update(event):
        ...
        if drag_line == 'axvline1':
            if xdata <= scnd_cx - max_thres/255:
                ...
                ax.set_title(f'{round(xdata, 3)} : {round(scnd_cx, 3)}', fontsize=12)
                fig.canvas.draw_idle()

            else:
                axvline1.set_xdata(scnd_cx - max_thres/255)
                ax.set_title(f'{round(scnd_cx - max_thres/255, 3)} : {round(scnd_cx, 3)}', fontsize=12)
                fig.canvas.draw_idle()
        # Move a segunda linha
        elif drag_line == 'axvline2':
            if xdata >= frst_cx + max_thres/255:
                axvline2.set_xdata(xdata)
                ax.set_title(f'{round(frst_cx, 3)} : {round(xdata, 3)}', fontsize=12)
                fig.canvas.draw_idle()
            else:
                axvline2.set_xdata(frst_cx + max_thres/255)
                ax.set_title(f'{round(frst_cx, 3)} : {round(frst_cx + max_thres/255, 3)}', fontsize=12)
                fig.canvas.draw_idle()

    # Quando soltar o botão do mouse
    def end_drag(event):
        global arr, x_zoom, y_zoom, arr_thres
        fig.canvas.mpl_disconnect(fig.canvas.mpl_connect('motion_notify_event', drag_update))
        fig.canvas.mpl_disconnect(fig.canvas.mpl_connect('button_release_event', end_drag))
        ...
        ax.set_title(f'{round(histo_min, 3)} : {round(histo_max, 3)}', fontsize=12)

        subprocess.call(['gdal_translate.exe', '-ot', 'Byte', '-quiet' ,'-scale', str(histo_min), str(histo_max), img_in, histo_dir + dir + file + '_Histo' + index])
    ...
    subprocess.call(['gdal_translate.exe', '-ot', 'Byte', '-quiet', '-scale', str(min), str(max), img_in, histo_dir + dir + file + '_Histo' + index])
    ...
    def crop_image_index(x, y):
        try:
            cropped = arr_thres[y-size_thres_y:y+size_thres_y, x-size_thres_x:x+size_thres_x]
    ...
    def crop_image_merge(x, y):
        try:
            merge_cropped = merge[y-size_merge_y:y+size_merge_y, x-size_merge_x:x+size_merge_x]
            resized_merge_cropped  = cv2.resize(merge_cropped, (size_merge_x, size_merge_y))
    ...
    def CallBackFunc_index(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_RBUTTONDOWN:
            ...
            fig.canvas.mpl_disconnect(fig.canvas.mpl_connect('motion_notify_event', drag_update))
            fig.canvas.mpl_disconnect(fig.canvas.mpl_connect('button_release_event', end_drag))
            fig.canvas.mpl_disconnect(fig.canvas.mpl_connect('close_event', on_close))
            ...
            num_index = (f'{str(histo_min)} : {str(histo_max)}')
            func(file, num_index)

            ap.histogram_image_save(file, path, index, dir, float(histo_min), float(histo_max))
            ...
            if event == cv2.EVENT_MOUSEMOVE:
                if dragging == True:
                    ...
                    if os.path.isfile(files_dir + file + '\\' + file + '.VNIR_Swath.ImageData2.tif') == True:
                        x =  int(x * merge_array.shape[1] / index_array.shape[1])
                        y =  int(y * merge_array.shape[0] / index_array.shape[0])
                        zoom_merge(x, y)
        except:
            return
    ...
    n,bins,patches = ax.hist(Z, bins=256, ec='white', fc='none', histtype='step', density=True)
