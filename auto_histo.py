...
for file in files:
    while (control < 3):
        ...
        min_1, max_1 = ap.histogram.get_histogram_range(img_in)
        subprocess.call(['gdal_translate.exe', '-ot', 'Byte', '-quiet', '-scale', str(min_1), str(max_1), img_in, f"Teste\\{file}original_{type}.tif"])
        ...
        cv2.namedWindow(file + f"_Original_{type}", cv2.WINDOW_NORMAL)
        cv2.namedWindow(file + f"_Processed_{type}", cv2.WINDOW_NORMAL)

        cv2.imshow(file + f"_Original_{type}", img)
        cv2.imshow(file + f"_Processed_{type}", img2)
